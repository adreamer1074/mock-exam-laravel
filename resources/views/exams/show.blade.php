            ...
            <form method="POST" action="{{ route('exam.submit', $exam->id) }}">
                ...
                @foreach ($exam->questions as $question)
                    <div class="question mb-8 p-6 bg-white shadow-lg rounded-lg">
                        <h3 class="text-xl font-semibold text-gray-800">Q{{ $loop->iteration }}. {{ $question->question_text }}</h3>
                        
                        {{-- 選択肢の表示 --}}
                        @if ($question->options->count() > 4)
                            {{-- 5つ以上の選択肢がある場合はチェックボックスを表示 --}}
                            <div class="mt-4">
                        ...
                        @else
                            ...
                            <div class="mt-4">
                                @foreach ($question->options as $option)
                                    <label class="block mt-2">
                                        <input type="radio" name="answers[{{ $question->id }}]" value="{{ $option->id }}" class="mr-2">
                                        {{ $option->option_text }}
                                    </label>
                                @endforeach
                            </div>
                ...
                <button type="submit" class="mt-4 bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">Submit Exam</button>
            </form>
        </div>
    </section>
